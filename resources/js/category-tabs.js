document.addEventListener('DOMContentLoaded', function() {
    // Get all instances of the containers (both desktop and mobile)
    const tabsContainers = document.querySelectorAll('#categoryTabsContainer');
    const postsContainers = document.querySelectorAll('#posts-container');
    const categoryLinks = document.querySelectorAll('.category-link');
    
    // Setup functionality for each tabs container (desktop and mobile)
    tabsContainers.forEach(tabsContainer => {
        const scrollRightButton = tabsContainer.parentElement.querySelector('.scroll-right');
        const scrollLeftButton = tabsContainer.parentElement.querySelector('.scroll-left');
    ...
    categoryLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            // Update active state for all category links (both desktop and mobile)
            ...
            // Find all links with the same data attributes and activate them
            const url = this.getAttribute('data-url');
            const category = this.getAttribute('data-category');
            
            document.querySelectorAll(`[data-category="${category}"]`).forEach(el => {
                el.classList.add('active', 'text-white', 'bg-blue-600');
                el.classList.remove('hover:text-gray-900', 'hover:bg-gray-100');
            });
